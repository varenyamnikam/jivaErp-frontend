...
export default function Employee() {
  ...
  if (employeeType.length == 0) {
    ...
    axios
      ...
      .then((response) => {
        ...
        setEmployeeType(
          response.data.acTypes.filter(
            (item) => "empType" in item && item.empTypeStatus == "ACTIVE"
          )
        );
      });
  }
  if (departmentType.length == 0) {
    ...
    axios
      ...
      .then((response) => {
        console.log("req sent Config.getacadress");
        setDepartmentType(response.data.deptTypes);
      });
  }
