  ...
  if ((records[0] && records[0].vouNo == "X X X X") || refresh) {
    ...
    axios
      ...
      .then((response) => {
        ...
        let deliveredItems = response.data.inv_voucherItems.filter(
          (item) => item.docCode == deliveryDocCode
        );
        let orederedItems = response.data.inv_voucherItems.filter(
            ...
            if (arr.length != 0) {
              ...
              arr.map((item) => {
                report.push({
                  ...
                  gst: calc(item).tot,
                  gstP: calc(item).totP,
                  deliveredQty: Number(getGr(deliveredItems, deliveredGr)),
